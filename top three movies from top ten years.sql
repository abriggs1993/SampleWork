...
FROM [Movies].[dbo].[Movies]
WHERE YEAR(release_date) BETWEEN 1993 AND 1995 OR YEAR(release_date) BETWEEN 1997 AND 2000 OR YEAR(release_date) BETWEEN 2019 AND 2021 --filter using the top to years found in previous query
GROUP BY YEAR(release_date), title
...
WITH cte AS  --create the cte
 (SELECT YEAR(release_date) AS year_released,
	...
	ROUND(AVG(vote_average),2) AS avg_rating
FROM [Movies].[dbo].[Movies]
WHERE YEAR(release_date) BETWEEN 1993 AND 1995 OR YEAR(release_date) BETWEEN 1997 AND 2000 OR YEAR(release_date) BETWEEN 2019 AND 2021
GROUP BY YEAR(release_date), title),
 ...
 topthree AS  --create another cte to get a ranking for each movie's ranking per year
	...
	ORDER BY year_released, avg_rating DESC) AS ranking --order by year and then by avg rating
	FROM cte )
